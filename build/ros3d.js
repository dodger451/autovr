...
ROS3D.Particles = function(options) {
  ...
  this.alphas = new Float32Array( this.max_pts );

  this.sizes = new Float32Array( this.max_pts );
  for(var i=0;i<this.max_pts;i++){
      this.sizes[i] = size;
  }
  ...
  this.geom.addAttribute( 'customColor', new THREE.BufferAttribute( this.colors, 3 ) );
...
function read_point(msg, index, data_view){
    ...
    for(var fi=0; fi<msg.fields.length; fi++){
        ...
        }else if( msg.fields[fi].name === 'ring' ){
          pt[ 'ring' ] = data_view.getUint16(si, 1);
        }else if( msg.fields[fi].name === 'class' ){
...
ROS3D.PointCloud2 = function(options) {
  ...
  this.rosTopic.subscribe(function(message) {
    ...
    if(message.data.buffer){
      // buffer = message.data.buffer.buffer; //double buffer?? fritz
    ...
    for ( var i = 0, i3 = 0; i < n; i ++, i3 += 3 ) {
      ...
      that.particles.positions[ i3 + 0 ] = pt['x'];
      that.particles.positions[ i3 + 1 ] = pt['y'];
      that.particles.positions[ i3 + 2 ] = pt['z'];
      // var color = new THREE.Color( pt['rgb'] );
      var value = ((((pt['z'])/5.0)%1)+1)%1;
      // var value = (pt['ring']*360.0)/64.0;
      color.setHSL(value,1,0.5);
      that.particles.colors[ i3 + 0 ] = color.r;
      that.particles.colors[ i3 + 1 ] = color.g;
      that.particles.colors[ i3 + 2 ] = color.b;
      that.particles.alphas[i] = 1.0;
    }
